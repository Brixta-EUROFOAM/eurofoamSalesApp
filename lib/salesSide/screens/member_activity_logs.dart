//lib/salesSide/screens/member_activity_logs.dart
...
import 'package:salesmanapp/salesSide/screens/edit_pjp_wizard_screen.dart';
...
class _PjpLogsTabState extends State<_PjpLogsTab> {
  ...
  void _approveAllGlobal(List<DailyTask> list) async {
    // 1. Get all pending task IDs
    final pendingTasks = list.where((t) => t.status.toLowerCase() == 'pending').toList();
    if (pendingTasks.isEmpty) return;

    final taskIds = pendingTasks.map((t) => t.id!).toList();
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("Approving ${pendingTasks.length} PJPs...")),
    );

    // 3. One single network request
    try {
      await widget.apiService.bulkUpdateDailyTaskStatus(taskIds, 'Approved');

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("All PJPs successfully approved!")),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Bulk approval failed. Syncing with server...")),
        );
        _refreshPjps(); // Revert UI if the server failed
      }
    }
  }
  ...
  void _approveAll(List<DailyTask> tasks) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Group visits approved")),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Approval failed. Syncing...")),
        );
        _refreshPjps();
      }
    }
