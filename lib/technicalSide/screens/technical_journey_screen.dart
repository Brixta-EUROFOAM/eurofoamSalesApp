...
class _TechnicalJourneyScreenState extends State<TechnicalJourneyScreen> {
  ...
  // ✅ Map State Flags (Prevents Crash)
  ...
  static const _initialCameraPosition = CameraPosition(target: LatLng(26.1445, 91.7362), zoom: 12);
  ...
  void _processNewJourneyData(Map<String, dynamic> journeyData) async {
    ...
    // Reset Route UI
    _routeTaken.clear();
    _isRouteLineLayerAdded = false; // Force redraw of new route
  ...
  Future<void> _startJourney() async {
    ...
    // Reset travelled line flag so it redraws fresh
  ...
  Future<void> _stopJourney() async {
    ...
    try {
      if (_lastRecordedPosition != null && _currentJourneyId != null) {
        ...
        await _apiService.sendGeoTrackingPoint(finalTrackingPoint);
      }

      if (_currentPjpId != null) {
        await _apiService.updatePjp(_currentPjpId!, {
          'status': 'COMPLETED',
          'actual_end_time': DateTime.now().toIso8601String()
        });
      }
      
      if (widget.onJourneyCompleted != null && _currentPjp != null && _currentSite != null) {
        widget.onJourneyCompleted!(_currentPjp!, _currentSite!, checkInTime);
      }

    } catch (e) {
      dev.log("❌ Error in stop journey: $e", name: 'TechJourney');
    }
  ...
  void _startLocationStream() {
    // ⚠️ Reduced accuracy to prevent Emulator Crash (DeadSystemException)
    const LocationSettings locationSettings = LocationSettings(
      accuracy: LocationAccuracy.high, 
      distanceFilter: 10
    );
    _positionStreamSubscription = Geolocator.getPositionStream(locationSettings: locationSettings).listen(_onPositionUpdate, onError: (e) => dev.log("Loc Error: $e"));
  }
  ...
  void _onPositionUpdate(Position position) {
    ...
    if (_lastRecordedPosition != null) {
      final double movement = Geolocator.distanceBetween(_lastRecordedPosition!.latitude, _lastRecordedPosition!.longitude, position.latitude, position.longitude);
      if (movement > 2.0) {
    ...
    if (_destinationLocation != null && !_isNearDestinationNotified) {
      final double dist = Geolocator.distanceBetween(position.latitude, position.longitude, _destinationLocation!.latitude, _destinationLocation!.longitude);
      if (dist < 500) { _showNearArrivalNotification(); _isNearDestinationNotified = true; }
    }

    if (mounted) setState(() => _distanceDisplay = "${(_totalDistanceTravelled / 1000.0).toStringAsFixed(2)} km");
  }

  // --- ✅ CRITICAL FIX: UPDATE SOURCE INSTEAD OF REMOVING ---
  Future<void> _drawUserLocationPointer(LatLng point) async {
    ...
    if (_isUserLocationLayerAdded) {
      ...
      try {
        ...
        // If it failed because it exists, flip the flag and try update next time
        _isUserLocationLayerAdded = true; 
      }
    }
  }

  // --- ✅ CRITICAL FIX: UPDATE POLYLINE INSTEAD OF REMOVING ---
  ...
  Future<void> _getDirectionsAndDrawRoute() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final geoJson = {
          'type': 'FeatureCollection',
          'features': [{'type': 'Feature', 'properties': {}, 'geometry': {'type': 'LineString', 'coordinates': routePoints.map((p) => [p.longitude, p.latitude]).toList()}}]
        };
  ...
  Future<void> _showNearArrivalNotification() async {
    ...
    await flutterLocalNotificationsPlugin.show(0, 'Approaching Site', 'You are close to the site.', const NotificationDetails(android: androidDetails));
  }
  ...
  void _showDestinationArrivalNotification() {
    ...
    _stopJourney();
    showDialog(context: context, barrierDismissible: false, builder: (context) => AlertDialog(title: const Text('SITE REACHED'), content: const Text('You have arrived.'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('OK'))]));
  }
  ...
  Future<void> _determinePositionAndMoveCamera() async {
    ...
    try {
      ...
      if (status != 'GRANTED_BACKGROUND' && status != 'GRANTED_FOREGROUND') { setState(() => _distanceDisplay = 'Permission Denied'); return; }
      ...
      // Call drawing function (which is now safe)
      _drawUserLocationPointer(_currentUserLocation!);
    } catch (e) { if (mounted) setState(() => _distanceDisplay = "Loc Error"); }
  }

  Future<String> _readStyle() async {
    if (_stadiaApiKey == null) throw Exception("API Key Missing");
    return jsonEncode({"version": 8, "sources": {"stadia": {"type": "raster", "tiles": ["https://tiles.stadiamaps.com/tiles/alidade_smooth_dark/{z}/{x}/{y}@2x.png?api_key=$_stadiaApiKey"], "tileSize": 256}}, "layers": [{"id": "stadia-layer", "source": "stadia", "type": "raster", "minzoom": 0, "maxzoom": 22}]});
  }
  ...
  Widget build(BuildContext context) {
    final bool canStartJourney = _destinationLocation != null && !_isJourneyActive && _currentSite != null;

    return Stack(
      children: [
        SizedBox.expand(child: FutureBuilder<String>(future: _styleFuture, builder: (ctx, snap) {
          if (!snap.hasData) return Container(color: _bgLight);
          return MapLibreMap(styleString: snap.data!, initialCameraPosition: _initialCameraPosition, onMapCreated: (c) { if (!_controllerCompleter.isCompleted) _controllerCompleter.complete(c); });
        })),
        Positioned(top: 50, right: 16, child: Container(decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: IconButton(icon: Icon(Icons.my_location, color: _cardNavy), onPressed: _determinePositionAndMoveCamera))),
  ...
  Widget _buildIdleJourneyPanel(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Icon(Icons.business, color: _textGrey, size: 20), const SizedBox(width: 8), Text("SELECTED SITE", style: TextStyle(fontWeight: FontWeight.w900, color: _textGrey, fontSize: 12))]),
      const SizedBox(height: 16),
      Container(height: 56, padding: const EdgeInsets.symmetric(horizontal: 4), decoration: BoxDecoration(color: _bgLight, borderRadius: BorderRadius.circular(16)), child: Center(child: TextField(controller: _destinationController, readOnly: true, style: TextStyle(color: _textDark, fontWeight: FontWeight.w600), decoration: InputDecoration(prefixIcon: Icon(Icons.lock_outline, color: _textGrey), hintText: "Waiting for PJP...", border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14))))),
    ]);
  }

  Widget _buildActiveJourneyPanel(BuildContext context) {
    return Column(children: [
      Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: _cardNavy, borderRadius: BorderRadius.circular(24)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("DISTANCE", style: TextStyle(color: Colors.white60, fontSize: 10)), const SizedBox(height: 8), Text(_distanceDisplay, style: const TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.bold))])),
        InkWell(onTap: _launchGoogleMapsNavigation, borderRadius: BorderRadius.circular(50), child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white12, shape: BoxShape.circle), child: const Icon(Icons.near_me_rounded, color: Colors.white, size: 28))),
      ])),
      const SizedBox(height: 16),
      Row(children: [Icon(Icons.flag_rounded, color: _textDark, size: 18), const SizedBox(width: 8), Expanded(child: Text(_destinationController.text.toUpperCase(), style: TextStyle(fontWeight: FontWeight.bold, color: _textDark, fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis))]),
    ]);
  }
}
